...
function FavoriteSection({ title, icon }: SectionProps) {
  const [type, setType] = useState<MediaType>("movie");
  const { user } = useUser();

  const { favorites, isLoadingFavorites } = useFavorites(user?.id ?? "", type);

  if (isLoadingFavorites) return <PageLoader message="Loading Profile" />;

  const myFavorites = favorites?.filter(
    (favorite) => favorite.media_type === type,
  );
      ...
      >
        {myFavorites?.length ? (
          myFavorites.map((favorite) => (
            <MediaItem
              key={`${favorite.media_type}-${favorite.media_id}`}
              media_id={favorite.media_id}
              media_type={favorite.media_type as MediaType}
            />
          ))
        ) : (
